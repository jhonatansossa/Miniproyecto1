...
##################                    ####################################
##########################################################################  
def formants_praat(audio_filename,sizeframe=0.025,step=0.01):
...
##########################################################################
#####                                                             ########
#####     F0 contour Voiced/unvoiced offset/onset transitions     ########
#####                                                             ########
##########################################################################  
#def f0_praat(audio_filename,data_audio,fs,sizeframe=0.04,step=0.01,minf0=75, maxf0=500):  
#    """f0 contour
#    """
...
#    
#    praat_vuv(audio_filename, temp_filename_f0, temp_filename_vuv, 
#                              time_stepF0=step, minf0=minf0, maxf0=maxf0)
#    f0,_ = decodeF0(temp_filename_f0,len(data_audio)/float(fs),step)
...
def f0_praat(audio_filename,data_audio,fs,sizeframe=0.04,step=0.01,minf0=75, maxf0=500):  
    """f0 contour.
    """
    temp_filename_f0=path_base+'/tempF0.txt'
    np.savetxt(temp_filename_f0,np.zeros((3,3))) 
    temp_filename_vuv=path_base+'/tempvuv.txt'
    np.savetxt(temp_filename_vuv,np.zeros((3,3)))  
    
    praat_vuv(audio_filename, temp_filename_f0, temp_filename_vuv, 
                              time_stepF0=step, minf0=minf0, maxf0=maxf0)
    f0,t = decodeF0(temp_filename_f0,len(data_audio)/float(fs),step)
    os.remove(temp_filename_f0)    
    os.remove(temp_filename_vuv)   
    return f0,t

def decodef0_transitions(sig,fs,f0,trflag,sztr=0.16,step=0.01):
    """
    F0 is the pitch contourn
    trflag = 1 for Onset, -1 for Offset
